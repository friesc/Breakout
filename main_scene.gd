...
@onready var paddelSpawnPosition = $PaddleSpawnPosition

func spawn_paddle()-> void:
	var paddle_instance := paddle.instantiate() as Paddle;
	paddle_instance.global_position = paddelSpawnPosition.global_position
...
func spawn_blocks()-> void:
	...
	for c in range(num_columns):
		for r in range(num_rows):
			...
			block_instance.global_position.x = HOR_PADDING + col_leftover_padding + block_size.x*0.5 + (c * (block_size.x+HOR_PADDING))
